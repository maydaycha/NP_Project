...
void str_echo(int sockfd){
    ...
    void do_command(char *argv,int sockfd, bool isfirst, bool islast, bool need_jump, int jump_num, bool show_error, char* fullCmdName){
        ...
        fullCmdName[strlen(fullCmdName)] = '\0';
        //fullCmdName[strlen(fullCmdName)-2] = '\0';
        //fullCmdName[strlen(fullCmdName)-3] = '\0';
        ...
        if((childpid = fork())<0){
            ...
            if(pipe_from_user){
                ...
                sprintf(msg, "*** %s (#%d) just received from %s (#%d) by '%s' ***\n", receiverName, client_own_id, senderName, senderID, fullCmdName);
                broadcast(msg);
            }
            if(pipe_to_user){
                char msg[100];
                ...
                sprintf(msg, "*** %s (#%d) just piped '%s' to %s (#%d) ***\n", senderName, client_own_id, fullCmdName, receiverName, receiverID);
                broadcast(msg);
            }
            return;
        }
            ...
            if(pipe_to_user){
                if(mkfifo( path, S_IFIFO | shmflg  )==-1){
                    char maymsg[50];
                    sprintf(maymsg, "error! make fifo : %s\n", path); 
                    err_dump(maymsg);
                }
                fprintf(stderr, "$$$$$$$$$$$$$$$$mkfifo success!: %s $$$$$$$$$$$$$$$$$\n",path);
                fprintf(stderr, "fifo path: %s\n", path);
                int writefd = open( path, O_WRONLY, S_IRWXU );
                dup2(writefd, STDOUT_FILENO);
                close(writefd);
                fprintf(stderr, "#%d, this child create FIFO\n", client_own_id);
                fprintf(stderr, "#%d, stdout is FIFO: %d\n", client_own_id ,writefd);
            }

            if(pipe_from_user){
                int readfd = open(path2, 0);
                if(readfd<0)
                    err_dump("$$$$$$$$$$$$$$$$$$$open fifi error$$$$$$$$$$$$$$$$$$$");
                dup2(readfd, STDIN_FILENO);
                close(readfd);
                fprintf(stderr, "#%d, this stdin is from FIFO: %d\n",client_own_id ,readfd);
                if(unlink(path2)<0){
                    char maymsg[50];
            ...
            int b=0;
            while(cmd[b]!=NULL)
                fprintf(stderr, "exe cmd : %s, ", cmd[b++]);
