...
#define MAX_WORD 500   // number of word in one line (cut by pipe)
#define MAX_USER 31
...
int main(int argc, char *argv[]){
    ...
    listen(sockfd,5);
    fprintf(stderr, "---------server start-----prot: %d-----\n",PORT);
    for (;;) {
...
void str_echo(int sockfd){
    ...
    for(;;) {
        ...
        if (n < 0) err_dump("str_echo: readline error");
        else if (n==0) return;    //terminate
        else if (n==2) continue;   // 防止按enter導致斷線
    ...
    void do_command(char *argv,int sockfd, bool isfirst, bool islast, bool need_jump, int jump_num, bool show_error, char* fullCmdName){
        ...
        if((childpid = fork())<0){
            ...
            if(pipe_to_user){
                ...
                shareptr->Blocking = true;
                kill(shareptr->PidTable[receiverID], SIGUSR2);
               // while(shareptr->Blocking)
                    usleep(1000);
                int writefd = open(path, O_WRONLY );
                fprintf(stderr,"!!!!!!!!!!!writer open fifo!!!!!!!\n");
                if(writefd == -1){
                    char maymsg[50];
